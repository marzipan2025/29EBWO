 ...
 *     꺼내 쓸 수 있으므로 넣지 않는다.
 *  2. 새 APK 가 지금 깔린 것과 **같은 키**로 서명돼 있어야 한다. 이 앱은 디버그 키
 *     (SHA-1 9F:26:8E…E3:32)로 서명한다. 다르면 안드로이드가 덮어쓰기를 거부하고,
 *     지우고 다시 깔면 받아 둔 글과 읽던 자리가 사라진다.
 *
...
object Updater {
    ...
    private const val API = "https://api.github.com/repos/marzipan2025/29EBWO/releases/latest"
    ...
    suspend fun download(ctx: Context, r: Release, onProgress: (Int) -> Unit): File? =
        withContext(Dispatchers.IO) {
            ...
            val out = File(dir, "29EBWO-${r.version}.apk")
            var ok = false
            try {
